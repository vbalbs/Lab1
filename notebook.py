...
def show_map2(graph_data, node_colors = None, edges = []):
	G = nx.Graph(graph_data['graph_dict'])
	node_colors = node_colors or graph_data['node_colors']
	node_positions = graph_data['node_positions']
	node_label_pos = graph_data['node_label_positions']
	...
	# draw the graph (both nodes and edges) with locations from romania_locations
	nx.draw(G, pos={k: node_positions[k] for k in G.nodes()},
			node_color=[node_colors[node] for node in G.nodes()], linewidths=0.3, edgecolors='k', edgelist=edges, node_size=150)
	...
	# add a legend
	white_circle = lines.Line2D([], [], color="white", marker='o', markersize=15, markerfacecolor="white")
	darkcyan_circle = lines.Line2D([], [], color="darkcyan", marker='o', markersize=15, markerfacecolor="darkcyan")
	red_circle = lines.Line2D([], [], color="red", marker='o', markersize=15, markerfacecolor="red")
	gray_circle = lines.Line2D([], [], color="gray", marker='o', markersize=15, markerfacecolor="gray")
	green_circle = lines.Line2D([], [], color="green", marker='o', markersize=15, markerfacecolor="green")
	fuchsia_circle = lines.Line2D([], [], color="fuchsia", marker='o', markersize=15, markerfacecolor="fuchsia")
	gold_circle = lines.Line2D([], [], color="gold", marker='o', markersize=15, markerfacecolor="gold")
	plt.legend((white_circle, darkcyan_circle, red_circle, gray_circle, green_circle, fuchsia_circle, gold_circle),
			   ('Un-explored', 'Frontier', 'Currently Exploring', 'Explored', 'Final Solution', 'Start', 'Goal'),
			   numpoints=1, prop={'size':16}, loc=(.7875,.05))
...
def final_path_colors2(initial_node_colors, problem, solution):
	"Return a node_colors dict of the final path provided the problem and solution."
	...
	final_colors[problem.initial] = "fuchsia"
	final_colors[problem.goal] = "gold"

	edges = [(problem.initial, solution[0])]
	for i in range(len(solution) - 1):
		edges.append((solution[i], solution[i + 1]))

	return (final_colors, edges)
...
def display_visual2(graph_data, user_input, algorithm=None, problem=None):
	initial_node_colors = graph_data['node_colors']
	if user_input == False:
		def slider_callback(iteration):
			# don't show graph for the first time running the cell calling this function
			try:
				show_map2(graph_data, node_colors=all_node_colors[iteration], edges=all_edges[iteration])
			except:
				pass
		def visualize_callback(Visualize):
			if Visualize is True:
				...
				iterations, all_node_colors, node = algorithm(problem)
				all_edges = [[]] * len(all_node_colors)
				solution = node.solution()
				nodes, edges = final_path_colors2(all_node_colors[-1], problem, solution);
				all_node_colors.append(nodes)
	...
	if user_input == True:
		...
		def slider_callback(iteration):
			# don't show graph for the first time running the cell calling this function
			try:
				show_map2(graph_data, node_colors=all_node_colors[iteration])
			except:
				pass
